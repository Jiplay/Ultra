...
-- Insert sample meals
INSERT INTO meals (user_id, name, meal_type, date, notes) VALUES
('demo-user', 'Healthy Breakfast', 'breakfast', CURRENT_DATE, 'Post-workout meal'),
('demo-user', 'Protein Lunch', 'lunch', CURRENT_DATE, 'High protein meal'),
('demo-user', 'Light Dinner', 'dinner', CURRENT_DATE, 'Light but nutritious'),
('demo-user', 'Post-Workout Snack', 'snack', CURRENT_DATE, 'Quick energy boost'),
('demo-user', 'Weekend Breakfast', 'breakfast', CURRENT_DATE - INTERVAL '1 day', 'Weekend treat'),
('demo-user', 'Quick Lunch', 'lunch', CURRENT_DATE - INTERVAL '1 day', 'Busy day meal')
ON CONFLICT DO NOTHING;

-- Insert sample meal items
-- Breakfast: Oats (100g) + Banana (150g) + Greek Yogurt (200g)
INSERT INTO meal_items (meal_id, item_type, item_id, quantity, notes) VALUES
(1, 'food', 8, 100, 'Base carbs'),      -- Oats
(1, 'food', 7, 150, 'Natural sweetness'), -- Banana
(1, 'food', 9, 200, 'Protein boost')      -- Greek Yogurt
ON CONFLICT DO NOTHING;

-- Lunch: Protein Power Bowl recipe (1 serving)
INSERT INTO meal_items (meal_id, item_type, item_id, quantity, notes) VALUES
(2, 'recipe', 1, 1.0, 'Full serving')     -- Protein Power Bowl
ON CONFLICT DO NOTHING;

-- Dinner: Salmon (150g) + Sweet Potato (200g) + Spinach (100g)
INSERT INTO meal_items (meal_id, item_type, item_id, quantity, notes) VALUES
(3, 'food', 4, 150, 'Grilled'),          -- Salmon
(3, 'food', 5, 200, 'Baked'),            -- Sweet Potato
(3, 'food', 6, 100, 'Steamed')           -- Spinach
ON CONFLICT DO NOTHING;

-- Snack: Almonds (30g) + Banana (120g)
INSERT INTO meal_items (meal_id, item_type, item_id, quantity, notes) VALUES
(4, 'food', 10, 30, 'Handful'),          -- Almonds
(4, 'food', 7, 120, 'Medium size')       -- Banana
ON CONFLICT DO NOTHING;

-- Weekend Breakfast: Greek Yogurt (250g) + Banana (180g)
INSERT INTO meal_items (meal_id, item_type, item_id, quantity, notes) VALUES
(5, 'food', 9, 250, 'Large portion'),    -- Greek Yogurt
(5, 'food', 7, 180, 'Large banana')      -- Banana
ON CONFLICT DO NOTHING;

-- Quick Lunch: Chicken Breast (200g) + Brown Rice (150g)
INSERT INTO meal_items (meal_id, item_type, item_id, quantity, notes) VALUES
(6, 'food', 1, 200, 'Grilled'),          -- Chicken Breast
(6, 'food', 2, 150, 'Cooked')            -- Brown Rice
ON CONFLICT DO NOTHING;